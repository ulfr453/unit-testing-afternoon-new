...
test('shortenTextshould not alter a string with less than 100 characters', () => {
    expect(shortenText(shortText)).toHaveLength(29)
})

test('shortenText should cut off extra characters after 100 and add three periods', () => {
    const shortended = shortenText(longText);
    expect(shortended).not.toHaveLength(longText.length);
    expect(shortended.slice(-3)).toBe(233);
})
